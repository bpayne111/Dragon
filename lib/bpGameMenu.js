//bpGamePresenter REQUIRES jquery-ui bp
var paused = false;
function pause(keepState) {
  if(!keepState) paused = !paused;
  return paused;
}

function isGameSaved() { //temp method of seeing if a loaded game exists

  if( JSON.parse(localStorage.getItem('player1')))
    {return true;} 
   return false;
}
...
showMenu.requires = function() {return 'jq'};
...
if (!isGameSaved()) {
  $('#loadGame').remove();
  console.log('no saved game so game removed');

} //remove load button if no game to load
